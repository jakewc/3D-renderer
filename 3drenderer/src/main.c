...
void setup(void) {
    ...
    float fov_x = atan(tan(fov_y / 2) * aspect_x) * 2; // the same as 180/3, or 60deg
    float z_near = 1.0;
    float z_far = 20.0;
...
void update(void) {
    ...
    for (int mesh_index = 0; mesh_index < get_num_meshes(); mesh_index++){
        ...
        for (int i = 0; i < num_faces; i++) {
            ...
            //after clipping, we need to break the polygon into triangles
            ...
            for (int t = 0; t < num_triangles_after_clipping; t++){
                ...
                float light_intensity_factor = -vec3_dot(normal, get_light().direction);
...
void render(void) {
    ...
    for (int i = 0; i < num_triangles_to_render; i++) {
        ...
        int rendering_method = get_render_method();
        // Draw filled triangle
        if (rendering_method == RENDER_FILL_TRIANGLE || rendering_method == RENDER_FILL_TRIANGLE_WIRE) {
        ...
        else if (rendering_method == RENDER_TEXTURED || rendering_method == RENDER_TEXTURED_WIRE) {
        ...
        else if (rendering_method == RENDER_WIRE || rendering_method == RENDER_WIRE_VERTEX || rendering_method == RENDER_FILL_TRIANGLE_WIRE || rendering_method == RENDER_TEXTURED_WIRE) {
            draw_triangle(
        ...
        else if (rendering_method == RENDER_WIRE_VERTEX) {
